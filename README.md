...
`Notification` = a real event generated by the booking/incident lifecycle
(booking approved/rejected, incident assigned/resolved), pushed live over
Server-Sent Events to anyone subscribed, and persisted so there's always
a real inbox/history even if nobody was online when it happened.

| Method | Endpoint | Description |
|---|---|---|
| `GET` | `/api/notifications?recipientEmail=` | A user's notification history, newest first |
| `POST` | `/api/notifications/{id}/read` | Mark one notification as read |
| `GET` | `/api/notifications/stream?recipientEmail=` | Live SSE stream — new notifications push instantly, no polling |
...
38/38 tests passing as of the latest run — unit tests for the service layer
